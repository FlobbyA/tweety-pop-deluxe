...
for f in subprocess.check_output(['git','ls-tree','-r','--name-only','039061b'],cwd=p,text=True).splitlines():
 if f!='index.html':assert (p/f).read_bytes()==subprocess.check_output(['git','show',f'039061b:{f}'],cwd=p),f
old=(p/'game-3.1.3.js').read_text();new=(p/'game-3.1.4.js').read_text()
...
assert 'game-3.1.4.js?v=3.1.4' in h
