...
function removeHelper(){bonusEgg=null;helperCooldown=levelConfig().helperGap}
...
function mascotStatus(mood,message="",priority=1){
 if(mascot.life>0&&priority<mascot.priority)return;
 mascot.mood=mood;mascot.moodLife=1.6;
 if(message&&(mascot.cooldown<=0||priority>mascot.priority)){
  mascot.message=message;mascot.life=1.7;mascot.cooldown=3.5;mascot.priority=priority;
 }
}
function buildLevel(n){
 if(n<1||n>LEVELS.length)throw new RangeError("Invalid level");
 awaitingContinue=false;preparing=false;levelStartScore=score;levelMaxCombo=0;helperClock=0;helperCooldown=0;bombFeedback=null;shells=[];activePointer=null;
...
function launchFlyer(){
 ...
 flyer={x:-55,y:165+Math.random()*120,vx:level===1?105:135,phase:0,dropped:false};text("TWEETY IS COMING!",240,118,true);sfx.bonus();mascotStatus("excited","Tweety’s coming!",2);assets.speakHelper(sound)
}
...
function showLevelClear(){
 ...
 assets.setScene("result");sfx.clear();mascotStatus("excited","Well done!",4);reaction="excited";
 ...
 document.getElementById("overlayCat").src=assets.url(assets.manifest.images.tweety_level_clear);
...
function update(dt){
 ...
 mascot.life=Math.max(0,mascot.life-dt);mascot.cooldown=Math.max(0,mascot.cooldown-dt);mascot.moodLife=Math.max(0,mascot.moodLife-dt);if(!mascot.life)mascot.priority=0;
 ...
 if(running&&!transition&&!clearPending){
   ...
   flyerTimer=Math.max(0,flyerTimer-dt);if(flyerTimer<=0&&!flyer&&!bonusEgg&&helperCooldown<=0){launchFlyer();flyerTimer=helperDelay()}
 }
 ...
 if(shot){
   ...
   if(bonusEgg&&Math.hypot(bonusEgg.x-shot.x,bonusEgg.y-shot.y)<r*1.9){
     let bx=bonusEgg.x,by=bonusEgg.y;removeHelper();score+=1500;scoreEl.textContent=String(score).padStart(6,"0");text("BEAM UP! +1500",bx,by,true);puff(bx,by,3,8);sfx.bonus();mascotStatus("excited","Room to breathe!",2);reaction="excited";reactionTimer=1.2;ceiling=Math.max(0,ceiling-18)
   }
...
function draw(){
 ...
 let portrait=awaitingContinue?assets.images.tweety_level_clear:!running&&!preparing&&ot.textContent==="Game Over"?assets.images.tweety_game_over:assets.images[`tweety_scorekeeper_${mood}`];if(portrait)character(portrait,57,668,65);
 x.save();x.textAlign="center";x.fillStyle="#ffe3a0";x.font="bold 10px system-ui";x.fillText("SCOREKEEPER",94,745);
 if(mascot.life>0){x.fillStyle="#fff0c9";x.fillRect(49,640,148,24);x.fillStyle="#332440";x.font="bold 12px system-ui";x.fillText(mascot.message,123,656);}x.restore();
...
document.getElementById("pauseBtn").onclick=()=>{if(!running)return;paused=!paused;document.getElementById("pauseBtn").textContent=paused?"Resume":"Pause";document.getElementById("pauseLabel").hidden=!paused;assets.setMusic(!paused);};
 ...
 document.getElementById("mainMenuBtn").onclick=()=>{reset();running=false;assets.stopCues();assets.setScene("menu");ot.textContent="Tweety Pop Deluxe";op.textContent="12 twilight stages. Match 3, drop clusters and let Tweety lend a paw.";document.getElementById("overlayCat").src=assets.url(assets.manifest.images.tweety_scorekeeper_neutral);startBtn.disabled=false;startBtn.textContent="Start Game";document.getElementById("menuSoundBtn").hidden=false;document.getElementById("mainMenuBtn").hidden=true;};
