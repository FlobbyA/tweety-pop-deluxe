...
function finish(win){if(!running&&!transition&&!clearPending)return;assets.setScene("result");document.getElementById("mainMenuBtn").hidden=false;document.getElementById("menuSoundBtn").hidden=true;document.getElementById("overlayCat").src=assets.url(assets.manifest.images[win?"tweety_level_clear":"tweety_game_over"]);running=false;transition=false;clearPending=false;if(!win)sfx.lose();overlay.classList.remove("hidden");ot.textContent=win?"TWEETY TAKES OVER!":"Game Over";op.textContent=win?`Score ${String(score).padStart(6,"0")} • Max combo x${maxCombo}`:`The eggs reached Tweety. Score ${String(score).padStart(6,"0")}.`;startBtn.textContent="Play Again"}
 ...
 if(clearPending&&!transition){
   if(grid.length===0&&!shot){
     ...
     if((visualsDone&&clearTimer>=.55)||clearTimer>=1.8){
       ...
       assets.setMusic(false);sfx.clear();text("LEVEL CLEAR!",240,300,true);
       const activeSession=session;
       advanceTimer=setTimeout(()=>{
         if(activeSession!==session)return;
         if(level<LEVELS.length){level++;buildLevel(level);prepareGameplay();text(`LEVEL ${level}`,240,330,true)}
         else finish(true)
       },750)
     }
   }else{
...
function prepareGameplay(){
 const token=session;running=false;assets.stopCues();assets.setScene("ready");
 ...
 assets.cue("sfx_get_ready",{maxWait:1800}).then(()=>{
  ...
  running=true;paused=document.hidden;last=performance.now();overlay.classList.add("hidden");startBtn.disabled=false;startBtn.textContent="Play Again";
