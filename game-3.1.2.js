...
function loop(now){let dt=Math.min(.056,(now-last)/1000);last=now;helperSafety(now);if(!document.hidden){while(dt>0){let step=Math.min(dt,1/120);update(step);dt-=step;}draw()}requestAnimationFrame(loop)}
