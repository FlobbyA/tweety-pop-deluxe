...
function buildLevel(n){
 ...
 for(let row=0;row<rows;row++)for(let col=0;col<cols;col++){
   let keep=n===1?(Math.random()<.72):(Math.random()<.82 && !(row>4&&col>2&&col<7));
...
function bg(){
 ...
 let by=55+ceiling;x.fillStyle=stompFlash>0?"#b18e62":"#6e5a43";x.fillRect(wall,by,W-wall*2,31);for(let xx=wall;xx<W-wall;xx+=58){x.fillStyle="#9b805d";x.fillRect(xx+3,by+3,52,25)}
...
function aimLine(){
 if(!running||shot||transition||clearPending)return;
...
function text(t,px,py,big=false){texts.push({t,x:px,y:py,life:1,big})}
...
function launchFlyer(){
 ...
 flyer={x:-55,y:165+Math.random()*120,vx:level===1?105:135,phase:0,dropped:false};text("BONUS IS COMING!",240,118,true);sfx.bonus()
}
function dropBonusEgg(){
 bonusEgg={x:240+Math.random()*100-50,y:250,vy:0,life:12};text("BONUS EGG!",240,160,true)
}
...
function snap(){
 ...
 if(matched){
   ...
   combo++;maxCombo=Math.max(maxCombo,combo);sfx.combo(combo);
 ...
 }else{combo=0;missStreak++}
 shots++;current=next;next=randColor();
 // Authentic-feeling pressure pulse: smooth stomp after a shot interval, faster on level 2.
 let every=level===1?8:5;if(shots%every===0){ceiling+=level===1?7:11;stompFlash=.35;shake=7;sfx.stomp();text("STOMP!",240,105,true)}
 if(!hatchTarget&&Math.random()<(level===1?.09:.13))triggerHatchCandidate();
 check()
}
function check(){
 if(grid.length===0){
...
function update(dt){
 ...
 if(running&&!transition&&!clearPending){
   ceiling += (level===1?0.38:0.82)*dt;
   flyerTimer-=dt;if(flyerTimer<=0&&!flyer){launchFlyer();flyerTimer=level===1?26:18}
 }
 ...
 if(shot){
   ...
   if(bonusEgg&&Math.hypot(bonusEgg.x-shot.x,bonusEgg.y-shot.y)<r*1.9){
     let bx=bonusEgg.x,by=bonusEgg.y;bonusEgg=null;score+=1500;scoreEl.textContent=String(score).padStart(6,"0");text("BONUS +1500",bx,by,true);sfx.hatch();ceiling=Math.max(0,ceiling-18)
   }
...
function drawFlyer(){
 if(!flyer)return;x.save();x.translate(flyer.x,flyer.y+Math.sin(flyer.phase)*7);
...
function draw(){
 ...
 texts.forEach(t=>{x.globalAlpha=Math.min(1,t.life*2);x.textAlign="center";x.font=`900 ${t.big?23:17}px system-ui`;x.lineWidth=5;x.strokeStyle="#241632";x.strokeText(t.t,t.x,t.y);x.fillStyle="#ffe276";x.fillText(t.t,t.x,t.y);x.globalAlpha=1});x.restore()
}
