...
function mascotStatus(mood,message="",priority=1){
 ...
 if(sound&&canVoice&&(important||Math.random()<.42)){
  ...
  if(list.length){lastVocal=list[Math.floor(Math.random()*list.length)];assets.play(lastVocal);vocalClock=7;lastImportant[role]=now;}
 }
}
function helperNeed(){
...
function reset(){session++;assets.stopCues();maxCombo=0;paused=false;score=0;level=1;scoreEl.textContent="000000";buildLevel(1)}
...
function snap(){
 ...
 }else{reaction="annoyed";reactionTimer=.7;combo=0;missStreak++;if(missStreak>=3)mascotStatus("annoyed","Hmm…",1)}
...
function update(dt){
 ...
 if(running&&!transition&&!clearPending){
   ...
   dangerClock-=dt;if(dangerClock<=0&&grid.some(e=>pos(e.row,e.col).y+r>470)){if(sound)assets.play("danger");mascotStatus("danger","Careful!",3);dangerClock=3;}
