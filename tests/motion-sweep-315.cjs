...
const results=[];
for(const hz of [60,90,120])for(const [name,aim] of Array.from({length:41},(_,i)=>{const angle=(-61.7+i*3)*Math.PI/180;return ['angle'+i,{x:240+Math.tan(angle)*428,y:200}]})){
 const pair=[];
 for(const file of ['game-3.1.4.js','game-3.1.5.js']){
...
fs.writeFileSync('docs/motion-sweep-315.json',JSON.stringify(results,null,2));console.log('PASS '+results.length+' angle/refresh A/B cases: identical attachment cells; faster travel');
