...
const s={window:{},document:{hidden:false,addEventListener:(k,f)=>listeners[k]=f},Audio,Image:class{set src(v){this.onload()}},fetch:()=>Promise.reject(Error('404')),setTimeout:f=>{timers.set(++tid,f);return tid},clearTimeout:id=>timers.delete(id),SpeechSynthesisUtterance:class{}};vm.createContext(s);vm.runInContext(fs.readFileSync('config/assets-3.0.2.js','utf8'),s);vm.runInContext(fs.readFileSync('assets-3.0.2.js','utf8'),s);return {s,a:s.window.TweetyAssets,instances,timers,listeners}}
...
await test('ready silences music and cue completion resolves',async({a,instances})=>{a.audioUnlock();a.setScene('ready');const p=a.cue('sfx_get_ready');assert(instances[0].paused);instances[2].emit('ended');await p;assert(instances[2].paused)});
await test('autoplay rejection resolves cue without blocking',async({a,instances})=>{a.audioUnlock();instances[2].reject=true;await a.cue('sfx_get_ready')});
await test('missing file resolves cue without blocking',async({a,instances})=>{a.audioUnlock();instances[2].error={code:4};assert.equal(await a.cue('sfx_get_ready'),false)});
...
await test('clear and lose use named external roles',({a,instances,timers})=>{a.audioUnlock();a.play('clear');assert.equal(instances[3].playCount,1);a.play('lose');assert(instances[3].paused);assert.equal(instances[4].playCount,1);for(const fn of [...timers.values()])fn()});
