...
const s={window:{},document:{hidden:false,addEventListener:(k,f)=>listeners[k]=f},Audio,Image:class{set src(v){this.onload()}},fetch:()=>Promise.reject(Error('404')),setTimeout:f=>{timers.set(++tid,f);return tid},clearTimeout:id=>timers.delete(id),SpeechSynthesisUtterance:class{}};vm.createContext(s);vm.runInContext(fs.readFileSync('config/assets-3.0.1.js','utf8'),s);vm.runInContext(fs.readFileSync('assets-3.0.1.js','utf8'),s);return {s,a:s.window.TweetyAssets,instances,timers,listeners}}
